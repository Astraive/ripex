...
impl<'a> Lexer<'a> {
    ...
    fn next_token(&mut self) -> Token {
        ...
        let token = match ch {
            ...
            'R' if self.scanner.peek_ahead(1) == Some('"') => return self.read_raw_string(start),
            'u' if self.scanner.peek_ahead(1) == Some('8')
            ...
            '#' => return self.read_preprocessor_line(start),
            '@' => {
                self.scanner.advance();
                TokenKind::At
            }
    ...
    fn read_raw_string(&mut self, start: Pos) -> Token {
        ...
        while let Some(ch) = self.scanner.peek() {
            if ch == ')' {
                let _saved = self.scanner.position();
                self.scanner.advance();
                let mut ok = true;
                ...
                if ok && self.scanner.advance_if_eq('"') {
                    break;
                }
                // reset
            }
        ...
        let value = self.scanner.slice(start).to_string();
        self.make_token_with_value(TokenKind::RawStringLit, start, value)
    }
    ...
    fn read_ustring(&mut self, start: Pos) -> Token {
        ...
        let value = self.scanner.slice(start).to_string();
        self.make_token_with_value(TokenKind::U16StringLit, start, value)
    }
    ...
    fn read_preprocessor_line(&mut self, start: Pos) -> Token {
        self.scanner.advance();
        while let Some(ch) = self.scanner.peek() {
            if ch == '\n' || ch == '\r' {
                break;
        ...
        let value = self.scanner.slice(start).to_string();
        self.make_token_with_value(TokenKind::Hash, start, value)
    }
