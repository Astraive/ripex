...
impl<'a> Lexer<'a> {
    ...
    fn next_token(&mut self) -> Token {
        ...
        let token = match ch {
            ...
            '\'' if self.scanner.peek_ahead(1) == Some('\'') => return self.read_char(start),
            'b' if self.scanner.peek_ahead(1) == Some('"')
                || self.scanner.peek_ahead(1) == Some('r') =>
            {
                self.scanner.advance();
                if self.scanner.peek() == Some('"') {
                    return self.read_byte_string(start);
                } else {
                    return self.read_raw_string(start);
                }
            }
            'r' if self.scanner.peek_ahead(1) == Some('#')
            ...
            '\'' => {
                ...
                self.scanner.advance();
                let word = self.scan_ident();
                return self.make_token_with_value(
                    TokenKind::Lifetime,
                    start,
                    format!("'{}", word),
                );
    ...
    fn skip_whitespace_and_comments(&mut self) {
        loop {
            ...
            match self.scanner.peek() {
                ...
                Some('/') if self.scanner.peek_ahead(1) == Some('*') => {
                    let start = self.scanner.position();
                    let is_doc = self.scanner.peek_ahead(1) == Some('*');
                    self.scanner.advance();
                    self.scanner.advance();
                    while let Some(ch) = self.scanner.peek() {
    ...
    fn read_byte_string(&mut self, start: Pos) -> Token {
        self.scanner.advance();
        while let Some(ch) = self.scanner.peek() {
            if ch == '"' {
                self.scanner.advance();
        ...
        let value = self.scanner.slice(start).to_string();
        self.make_token_with_value(TokenKind::ByteLit, start, value)
    }
