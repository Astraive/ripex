...
impl<'a> Lexer<'a> {
    ...
    fn next_token(&mut self) -> Token {
        ...
        let token = match ch {
            ...
            'L' if self.scanner.peek_ahead(1) == Some('"')
                || self.scanner.peek_ahead(1) == Some('\'') =>
            {
                self.scanner.advance();
                if self.scanner.peek() == Some('"') {
                    return self.read_lstring(start);
                } else {
                    return self.read_lchar(start);
                }
            }
            'u' if self.scanner.peek_ahead(1) == Some('"')
                || self.scanner.peek_ahead(1) == Some('\'') =>
            {
                self.scanner.advance();
                if self.scanner.peek() == Some('"') {
                    return self.read_ustring(start);
                } else {
                    return self.read_uchar(start);
                }
            }
            'U' if self.scanner.peek_ahead(1) == Some('"')
                || self.scanner.peek_ahead(1) == Some('\'') =>
            ...
            '#' if !self.in_preproc => {
                ...
                self.scanner.advance();
                let word = self.scan_ident();
                let _kind = match word.as_str() {
                    "include" | "define" | "undef" | "if" | "ifdef" | "ifndef" | "else"
                    | "elif" | "endif" | "error" | "pragma" | "line" => TokenKind::HashHash,
                    _ => TokenKind::Hash,
                };
                return self.make_token_with_value(TokenKind::Hash, start, word);
            }
            '#' => {
                self.scanner.advance();
                TokenKind::Hash
    ...
    fn read_lstring(&mut self, start: Pos) -> Token {
        self.scanner.advance();
        while let Some(ch) = self.scanner.peek() {
            if ch == '"' {
                self.scanner.advance();
        ...
        let value = self.scanner.slice(start).to_string();
        self.make_token_with_value(TokenKind::LStringLit, start, value)
    }

    fn read_ustring(&mut self, start: Pos) -> Token {
        self.scanner.advance();
        while let Some(ch) = self.scanner.peek() {
            if ch == '"' {
                self.scanner.advance();
        ...
        let value = self.scanner.slice(start).to_string();
        self.make_token_with_value(TokenKind::UStringLit, start, value)
    }
    ...
    fn read_lchar(&mut self, start: Pos) -> Token {
        self.scanner.advance();
        while let Some(ch) = self.scanner.peek() {
            if ch == '\'' {
                self.scanner.advance();
        ...
        let value = self.scanner.slice(start).to_string();
        self.make_token_with_value(TokenKind::LCharLit, start, value)
    }

    fn read_uchar(&mut self, start: Pos) -> Token {
        self.scanner.advance();
        while let Some(ch) = self.scanner.peek() {
            if ch == '\'' {
                self.scanner.advance();
        ...
        let value = self.scanner.slice(start).to_string();
        self.make_token_with_value(TokenKind::UCharLit, start, value)
    }
